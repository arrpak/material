...
# El paquete `ggmap` está pensado para representar mapas e información sobre mapas 
# con una sintaxis similar a la de `ggplot2`. El paquete contiene tres tipos de funciones:
# * Para descargar mapas de diversas fuentes
# * Para representar capas (o _geoms_) sobre dichos mapas.
# * Para utilizar recursos geográficos, como los de geolocalización, disponibles en 
# diversas APIs.

# Por defecto, se utilizan los servicios de Google Maps, que están sujetos a limitaciones 
# de uso. Pero es posible consultar otros.
...
carburantes <-read.table("/home/dsc/Downloads/data/carburantes_20050222.csv",header=TRUE,sep='\t',dec=',',quote='')

carburantes <-carburantes[carburantes$Provincia=='MADRID',]

head(carburantes)

colnames(carburantes)

coor <- data.frame(long<-as.numeric(carburantes$X.Longitud..WGS84..),lati <- as.numeric(carburantes$X.Latitud..WGS84..))
...
map.gas <- get_map(location = coor,
                      color = "color",
                      maptype = "terrain",
                      scale = 2,
                      zoom = 8 , #"auto" ) 
                      source =  "google") #  c("google", "osm","stamen", "cloudmade"))
...
map.gas <- get_openstreetmap (location = coor,
                   color = "color",
                   maptype = "terrain",
                   scale = 2,
                   zoom = 8 , #"auto" )  
                   ) #  c("google", "osm","stamen", "cloudmade"))


ggmap(map.gas) + geom_point(aes(x = long, y = lati),
                               data = coor, colour = 'blue',
                               size = 1)


# conversiín de coordenadas sexagesimales a decimales

latitudes <- c("40º2'30''","41º23'54''")

x <- unlist(strsplit(latitudes[1], split = "º"))
grados <- as.numeric(x[1])
y <- unlist(strsplit(x[2], split = "'"))
minutos <-as.numeric(y[1]) 
segundos <- as.numeric(y[2]) 

coord <- grados+(minutos/60)+(segundos/(60*60))

coord

conver <- function(pos)
{
  x <- unlist(strsplit(pos, split = "º"))
  grados <- as.numeric(x[1])
  y <- unlist(strsplit(x[2], split = "'"))
  minutos <-as.numeric(y[1]) 
  segundos <- as.numeric(y[2]) 
...
conver2 <- function(long,lat)
{
data.frame(conver(long),conver(lat))
}

print(conver2("5º3'45''","50º36'45''"))



latitudes <- c("40º2'30''","141º23'54''")
gsub("([0-9]*).([0-9]*).([0-9]*).*","\\1-\\2-\\3",latitudes)


ltmp <- gregexpr("[0-9]+",latitudes)
tmp
...
# Los `shapefiles` son estructuras de datos que combinan información geográfica (típicamente, 
# perímetros de regiones) con información estadística relativa a dichos elementos. 
# Un _shapefile_ no es un fichero sino, típicamente, un grupo de ellos que comparten 
# nombre, tienen diversas extensiones y que se leen conjuntamente. 

# En R, los _shapefiles_ pueden leerse con el paquete _rgdal_, que los convierte en 
# objetos S4. Los objetos S4 son unas extensiones de R para la programación orientada 
# a objetos, que es muy conveniente para procesar información de tipo geográfico. Pero 
# que exigen el uso de una (otra) sintaxis particular.

library(rgdal)
sport <- readOGR(dsn = "/home/dsc/Downloads/data/london_sport", "london_sport")
class(sport)
...
# En efecto, un objeto de la clase `SpatialPolygonsDataFrame` es una tabla (_dataframe_) 
# más una lista de _polígonos_, asociados a ella y que permite representarlos en un 
# mapa. Un `SpatialPolygonsDataFrame` permite realizar operaciones de filtrado, etc. 
# similares a las que ya sabemos hacer con tablas.
...
##### Ejercicio
# Añade una columna al objeto `sport` y represéntala gráficamente con la intensidad 
# según el valor asociado. Pista: visita 
# [esta página](http://cran.r-project.org/doc/contrib/intro-spatial-rl.pdf) y busca 
# el código que acompaña al mapa de interés.
...
##### Ejercicio (avanzado)
# Descargar los _shapefiles_ de provincias del INE, obtén datos de algún tipo de estadistica 
# por provincias (¿paro? ¿población?), crea un _SpatialPolygonsDataFrame_ con esa información 
...
# En esta sección vamos a explorar el paquete `igraph` para leer, procesar, extraer 
# información y representar gráficamente datos que tienen estructura de red. Para ello 
# usaremos información disponible en internet de `güifi.net`, un proyecto para crear 
# redes comunitarias de internet vía wifi.
...
# Luego bajaremos y procesaremos el fichero .xml que guarda información de la subred 
# de la zona de Barcelona.

tmp <- readLines("https://guifi.net/en/guifi/cnml/2435/detail")
tmp <- xmlParse(tmp)

# Con [XPath](https://es.wikipedia.org/wiki/XPath) podemos extraer la información de 
# los nodos para después poder extraer sus atributos.
...
# Finalmente, filtramos los nodos _redundantes_ (enlaces cuyos nodos no están en la 
# lista de nodos, etc.) y creamos el grafo.
...
g.wc <- subgraph(g.working, tmp$membership == which.max(tmp$csize))
# filtra los nodos

table(tmp$membership)

sort(degree(g.working))

hist(degree(g.working))
            

##### Ejercicio
# Explora el objeto `tmp`.

# Podemos enriquecer el gráfico con información calculada a partir del grafo, como 
# el _betweenness_: esto es el numero de geodesicas que pasan

g.wc <- set.edge.attribute(g.wc, name = "betweennessz", E(g.wc), 
                           edge.betweenness(g.wc))

# geodesica:ruta mas corta entre ps nodos
...
plot(g.wc, layout = geom.layout, vertex.label = NA, 
     vertex.size = (1 + V(g.wc)$btw) / 3000,
      edge.width = (1 + E(g.wc)$betweenness) /1000 )

# no funciona lo del ed.width
